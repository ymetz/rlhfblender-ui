...
export function BackendConfigSequenceVisualizer(
  props: BackendConfigSequenceVisualizerProps,
) {
  ...
  return (
    <Box>
      <Typography variant="body1">Preview of Experiment Sequence</Typography>
    ...
    for (let i = 0; i < uiConfigs.length; i++) {
      ...
      while (episode_counter < nrOfElements) {
        ...
        const batchSize = parseInt(
          String(uiConfig?.max_ranking_elements) || "0",
        ) as number;
        const currentEpisodeCounter = episode_counter;
        let elementsInBatch = Array.from(
          { length: batchSize },
          (_, k) => currentEpisodeCounter + k,
        ).filter((e) => e < nrOfElements);
        sequences.push({
          uiConfig: { id: uiConfig.id, name: uiConfig.name },
          batch: elementsInBatch,
        } as SequenceElement);
