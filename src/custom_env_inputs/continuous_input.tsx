...
export default function ContinuousActionInput(props: ContinuousActionInputProps) {
  const [actionValues, setActionValues] = React.useState<number[]>(Array(props.space.shape[0]).fill(0));

  console.log(props.space);

  // Initialize min and max values for each action dimension
  const actionBounds = Array.isArray(props.space.low) && Array.isArray(props.space.high) ? props.space.low.map((low, index) => ({
    min: low as number,
      ...
      <Stack spacing={2}>
        ...
        {actionBounds.map((bounds, index) => (
          <Box key={index} sx={{ width: '80%', margin: '0 auto' }}>
            <Typography variant="body2" align="center">{`Action ${index + 1}`}</Typography>
            <Slider
