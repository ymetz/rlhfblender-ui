import { TrajectorySegment, MergedSegment } from '../types/projectionTypes';

// Extract K-step segments from trajectories
export function extractSegments(episodeToPaths: Map<number, number[][]>, segmentSize: number = 50): TrajectorySegment[] {
...
export function calculateSegmentSimilarity(seg1: TrajectorySegment, seg2: TrajectorySegment): number {
...
export function mergeSegments(segments: TrajectorySegment[], similarityThreshold: number = 0.05): MergedSegment[] {
...
export function calculateConvexHull(points: number[][]): [number, number][] {
...
export function calculateSimpleConcaveHull(points: number[][], maxEdgeLength?: number): [number, number][] {
    if (points.length < 3) return points.map(p => [p[0], p[1]] as [number, number]);
